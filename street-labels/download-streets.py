...
if __name__ == '__main__':
    ...
    for tmp_table in tables:
        ...
        if rows:
            db.execute('''INSERT INTO %(next_table)s
                          SELECT highway, name, long_name, zoomlevel, pixelwidth, SetSRID((Dump(way)).geom, %(srid)d)
    ...
    db.execute('CREATE INDEX %(next_table)s_highways ON %(next_table)s (highway)' % locals())
    ...
    db.execute("DELETE FROM geometry_columns WHERE f_table_name = '%(next_table)s'" % locals())
