...
keys = [key for key in s3.list('%s/streets-geojson/streets-' % directory) if key.name.endswith('.json.bz2')]
...
curl -sOL http://s3.amazonaws.com/%(bucket)s/%(directory)s/process-streets.py

python process-streets.py %(bucket)s %(prefix)s 12 13 14 15
